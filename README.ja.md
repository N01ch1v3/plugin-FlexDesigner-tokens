...
- **Codex ステータスキー** — CLI ターンの状態（`WORKING`、`COMPLETE`、`ABORTED`）、`TESTING`、`EDITING`、`SEARCHING` などの処理カテゴリ、モデル・推論強度・サンドボックスをまとめて表示
...
- **Anthropic から要請があった場合、Claude キーの機能は予告なく無効化・変更することがあります**

**なぜこの方法しかないのか:** `~/.claude/projects/**/*.jsonl` に記録されるのは「消費した」トークン数のみで、レート制限枠やリセット時刻の情報が一切含まれません。そのため、正確な**残量**を知る手段が他に存在しません。

一方 **Codex 側はローカルファイルの読み取りのみ**で、認証もネットワークアクセスも不要です。ステータスとセッション情報は Codex 内部の rollout JSONL 形式に依存するため、Codex 側の形式変更時には追従が必要になる場合があります。ログが静かなだけでは入力待ちを確定できないため、`Requires input` としては表示しません。

ステータスキーは、Codex の surface を問わず最後に更新されたローカル rollout を追跡します。完全なディレクトリパスは表示せず、常に `cwd` の親ディレクトリ名をプロジェクト名として使用します。プロジェクト名は灰色の `CODEX` ラベルと区別できるよう白色で描画します。
...
1. [Releases](https://github.com/ari-show/plugin-FlexDesigner-tokens/releases) から、自分の OS / アーキテクチャに合った `.flexplugin` をダウンロード
...
src/
  ...
  providers/
    ...
    codexActivity.js     rollout JSONL からローカルターン状態とセッション情報を推定
com.arishow.aitokens.plugin/
  manifest.json          キー定義・多言語リソース（en / ja）
  ui/*.vue               設定画面（Vue 3 + Vuetify 3）
test/
  providers/*.test.js    純粋ロジック（normaliseWindow 等）の unit test
```
